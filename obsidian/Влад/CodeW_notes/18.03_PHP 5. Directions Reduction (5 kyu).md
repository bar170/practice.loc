...
Функция принимает массив `$arr` содержащий направления ЮГ СЕВЕР ЗАПАД ВОСТОК. Функция должна сократить рядом стоящие значения с противоположными сторонами света.

Решил подойти к задаче исключительно математически, но для этого пришлось добавлять новый элемент в массив для проверки его окончания и избежания ошибки с неверным ключем. 

Использованы: [[unset()]], [[array_values()]]